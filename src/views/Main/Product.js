...
export default class Product extends Component {
  render() {
    return (
      <div className="flex-center flex-wrap w-11/12 lg:w-3/4 gap-[14px] rounded-[20px]">
        {this.props.prods.map((prod, index) => {
          return (
            <div
              key={index}
              className={`w-10/12 md:w-[32%] flex-center flex-col gap-4 ${
                index >= 3 ? " mt-20" : ""
              }`}>
              <div className="relative w-full">
                <img alt="" src={prod.img} className="w-full" />
                <div className="absolute flex-center gap-1 bottom-[22px] left-[6px]">
